...
import { LayoutEngine, LayoutCanvas, createLayoutEngine, LayoutElement } from '@lc-studio/layout-engine'
...
export default function LayoutEditor() {
  ...
  const handleAddElement = () => {
    ...
    engineRef.current?.addElement(newElement)
  }
  ...
  const handleDragStart = (id: string, e: MouseEvent) => {
    ...
    engineRef.current?.startDrag(id, { x: startX, y: startY })
  }

  const handleDragMove = (e: MouseEvent) => {
    engineRef.current?.updateDrag({ x: e.clientX, y: e.clientY })
  }

  const handleDragEnd = () => {
    engineRef.current?.endDrag()
    const updatedElements = engineRef.current?.getAllElements()
    if (updatedElements) {
      setElements(updatedElements as ElementData[])
    }
  }
  ...
  return (
    <div>
      <LayoutCanvas
        ...
        canvasWheel={handleDragMove}
        canvasDrop={handleDragEnd}
      />
    </div>
  )
}
